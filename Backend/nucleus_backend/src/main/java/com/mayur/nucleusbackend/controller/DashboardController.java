...
public class DashboardController {
    ...
    public ResponseEntity<ApiResponse<DashboardResponse>> getOrganizationDashboard(
        ...
        try {
            ...
            DashboardResponse dashboard = dashboardService.getOrganizationDashboard(organizationId);

            return ResponseEntity.ok(ApiResponse.success("Organization dashboard data fetched successfully", dashboard));
        } catch (Exception e) {
            return ResponseEntity.badRequest()
                    .body(ApiResponse.error("Failed to fetch organization dashboard: " + e.getMessage()));
        }
    }
}